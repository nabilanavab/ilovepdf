...
"""
        ...
        bot.send_message(
            message.chat.id, hlpMsg, disable_web_page_preview=True, reply_markup=key
        )
        ...
        def helpMsgClose(call):
            ...
            if edit == "close":
                ...
                bot.delete_message(
                    chat_id=call.message.chat.id, message_id=message.message_id
                )
...
def pic(message):
    try:
        ...
        img = Image.open(f"./{message.chat.id}/imgs/{message.chat.id}.jpg").convert(
            "RGB"
        )
