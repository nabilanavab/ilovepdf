...
async def text_to_pdf(bot, callbackQuery):
    try:
        ...
        _, scale, h_font, p_font, color, background = [
            int(i) if i.isdigit() else 1
            for i in callbackQuery.data.replace(":", "").split("|")
        ]
        ...
        isSuccess, title = await ask_for_title(
            bot, callbackQuery=callbackQuery, text=CHUNK["askT"]
        )
        if not isSuccess:
        ...
        while nabilanavab:
            ...
            if not isSuccess:
                ...
                if (
                    TXT[callbackQuery.message.chat.id][0] == None
                    and len(TXT[callbackQuery.message.chat.id]) == 1
                ):
        ...
        if not BACKGROUND_L.get(background, False):
            background = await bot.download_media(
                message=background, file_name=f"{cDIR}/"
            )
        ...
        if TXT[callbackQuery.message.chat.id][0] != None:
            pdf.multi_cell(
                pdf.w - 40 if POSITION[0] == "w" else POSITION[0],
                POSITION[1],
                txt=get_display(reshape(TXT[callbackQuery.message.chat.id][0])),
                ln=True,
                align="C",
            )
        ...
        for para in TXT[callbackQuery.message.chat.id][1:]:
            ...
            if isinstance(para, str):
                pdf.multi_cell(
                    pdf.w - 20 if POSITION[2] == "w" else POSITION[2],
                    POSITION[3],
                    txt=get_display(reshape(f"     {para}")),
                    ln=True,
                    align="L",
                )
            if isinstance(para, dict):
                if para["type"] == "photo":
                    ...
                    link = (
                        ...
                        if para["caption"]
                        and re.match(
                            r"^(http|https|ftp)://[^\s/$.?#].[^\s]*$", para["caption"]
                        )
                        else ""
                    )
                    img = await bot.download_media(
                        message=para["id"], file_name=f"{cDIR}/"
                    )
                    with Image.open(img) as image, pdf.local_context(
                        blend_mode="Multiply"
                    ):
        ...
        FILE_NAME, FILE_CAPT, THUMBNAIL = await fncta.thumbName(
            callbackQuery.message, f"{callbackQuery.message.chat.id}.pdf"
        )
        if images.PDF_THUMBNAIL != THUMBNAIL:
            location = await bot.download_media(
                message=THUMBNAIL, file_name=f"{cDIR}/{callbackQuery.message.id}.jpeg"
            )
        ...
        _COFFEE, COFFEE = await util.translate(button="feedbackMsg['button']", lang_code=lang_code)
