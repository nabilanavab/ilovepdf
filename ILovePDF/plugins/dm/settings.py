...
file_name = "ILovePDF/plugins/dm/settings.py"
    ...
    try:
        ...
        if data == "B":  # Home|B will redirect to settings
            if not dataBASE.MONGODB_URI:
                ...
                if not data:  # if monogo data fetching fails
                    error, errorCB = await util.translate(
                        text="SETTINGS['error']",
                ...
                args = [
                    ...
                    data["join_date"],
                    await disLang(lang_code),
                    f"`{data['api']}`" if data.get("api", 0) else defalt[0],
                    defalt[1] if data.get("thumb", 0) else defalt[0],
                    f"`{data['capt']}`" if data.get("capt", 0) else defalt[0],
                    f"`{data['fname']}`" if data.get("fname", 0) else defalt[0],
                ]
                await callbackQuery.edit_message_media(
                    InputMediaPhoto(data["thumb"])
                    if data.get("thumb", 0)
        ...
        if data.startswith("lang"):  # getLang
            ...
            await callbackQuery.message.reply_text(
                text=_.format(await disLang(lang)), reply_markup=__
            )
        ...
        if not data.endswith(
            "+"
        ):  # for successful cbMGS, add wait for use & cb expires
            result, _ = await util.translate(
