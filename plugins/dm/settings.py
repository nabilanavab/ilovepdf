    ...
    try:
        ...
        if data == "B":        # Home|B will redirect to settings
            if not dataBASE.MONGODB_URI:
                defalt, _ = await util.translate(text="SETTINGS['default'][0]", lang_code=lang_code)
                args = [
                ...
                if not data:    # if monogo data fetching fails
                     error, errorCB = await util.translate(text="SETTINGS['error']", button="SETTINGS['back'][0]", lang_code=lang_code)
                     return await callbackQuery.edit_message_caption(caption=error, reply_markup=errorCB)
                
                defalt, _ = await util.translate(text="SETTINGS['default']", lang_code=lang_code)
                args = [
                    f"{callbackQuery.from_user.mention}", f"{callbackQuery.from_user.id}",
                    f"@{callbackQuery.from_user.username}" if callbackQuery.from_user.username else "❌",
            ...
            tTXT, tBTN = await util.translate(text="HOME['HomeB']", button="HOME['HomeBCB']", lang_code=lang_code)
        ...
        if data == "B2S":        # from settings part function to settings
            _, tBTN = await util.translate(button="HOME['HomeBCB']", lang_code=lang_code)
        ...
        if data.startswith("lang"):    # getLang
            if not set.MULTI_LANG_SUP:
                cant, _ = await util.translate(text="SETTINGS['cant']", lang_code=lang_code)
            ...
            if data == "thumb":
                if user_id in CUSTOM_THUMBNAIL_U:
                    _, tBTN = await util.translate(button="SETTINGS['thumb'][1]", order=121, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
                else:
                    _, tBTN = await util.translate(button="SETTINGS['thumb'][0]", order=111, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
            elif data == "thumb+":    # add thumb
                tTXT, tBTN = await util.translate(text="SETTINGS['ask']", button="SETTINGS['wait']", order=111, lang_code=lang_code)
            ...
            if data == "api":        # DATA saved order ["api", "fname", "caption]
                if current and current[0]:
                    _, tBTN = await util.translate(button="SETTINGS['api'][1]", order=121, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
                else:
                    _, tBTN = await util.translate(button="SETTINGS['api'][0]", order=111, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
            elif data == "api+":
                tTXT, tBTN = await util.translate(text="SETTINGS['ask']", button="SETTINGS['waitApi']", order=111, lang_code=lang_code)
                await callbackQuery.edit_message_caption(caption=tTXT[0], reply_markup=tBTN)
                _, __ = await util.translate(text="SETTINGS['askApi']", lang_code=lang_code)
                await asyncio.sleep(1)
            ...
            if data == "fname":        # DATA saved order ["api", "fname", "caption]
                if current and current[1]:
                    _, tBTN = await util.translate(button="SETTINGS['fname'][1]", order=121, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
                else:
                    _, tBTN = await util.translate(button="SETTINGS['fname'][0]", order=111, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
            elif data == "fname+":
                tTXT, tBTN = await util.translate(text="SETTINGS['ask']", button="SETTINGS['wait']", order=111, lang_code=lang_code)
                await callbackQuery.edit_message_caption(caption=tTXT[0], reply_markup=tBTN)
            ...
            if data == "capt":        # DATA saved order ["api", "fname", "caption]
                if current and current[2]:
                    _, tBTN = await util.translate(button="SETTINGS['capt'][1]", order=121, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
                else:
                    _, tBTN = await util.translate(button="SETTINGS['capt'][0]", order=111, lang_code=lang_code)
                    return await callbackQuery.message.edit_reply_markup(tBTN)
            elif data == "capt+":
                tTXT, tBTN = await util.translate(text="SETTINGS['ask']", button="SETTINGS['wait']", order=111, lang_code=lang_code)
                await callbackQuery.edit_message_caption(caption=tTXT[0], reply_markup=tBTN)
        ...
        if not data.endswith("+"):    # for successful cbMGS, add wait for use & cb expires
            result, _ = await util.translate(text="SETTINGS['result'][1]", lang_code=lang_code)
