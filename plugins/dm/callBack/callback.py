...
async def _pdf(bot, callbackQuery):
    try:
        ...
        if data == "rotate":
            ...
            tTXT, tBTN = await util.translate(button = "BUTTONS['filter']", order = 1221, lang_code = lang_code)
        elif data == "comocr":
        ...
        # encrypted/non encrypted input pdf file
            ...
            return await callbackQuery.message.edit(
                text = tTXT['passMSG'].format(callbackQuery.message.reply_to_message.document.file_name,   #password 300 char limit
                    await render.gSF(callbackQuery.message.reply_to_message.document.file_size),
                    input_str.text[:50] if (data == "aioInput|enc" and input_str) else None, None, None, None ),
